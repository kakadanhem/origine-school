...
class RelationshipController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create','create-quick', 'update', 'delete', 'pdf', 'save-as-new'],
                        'roles' => ['@']
                    ],
                    [
                        'allow' => false
    ...
    /**
     * Creates a new StudentGuardian model based on student & guardian.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreateQuick($sid,$gid)
    {
        $model = new StudentGuardian();
        $student = Student::find()->where(['id' => $sid])->one();
        $guardian = Guardian::find()->where(['id' => $gid])->one();

        if ($model->loadAll(Yii::$app->request->post()) && $model->saveAll()) {
            return $this->redirect(['index']);
        } else {
            return $this->render('createQuick', [
                'model' => $model,
                'student' => $student,
                'guardian' => $guardian,
            ]);
    ...
    /**
     * 
     * Export StudentGuardian information into PDF format.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
    * Creates a new StudentGuardian model by another data,
