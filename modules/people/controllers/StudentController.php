...
class StudentController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['custom-save','student-list','group-modal-save','index', 'view', 'create', 'update', 'delete', 'pdf', 'save-as-new', 'add-enrollment', 'add-studentguardian'],
                        'roles' => ['@']
                    ],
                    [
                        'allow' => false
    ...
    public function actionCustomSave($direct)
    {
        $model = new Student();

        if ($model->loadAll(Yii::$app->request->post(),['enrollments','studentguardians']) && $model->saveAll(['enrollments','studentguardians'])) {
            if($direct == 'create-guardian'){
                return $this->redirect(['/guardian/create']);
            }
            else{
                return $this->redirect(['view', 'id' => $model->id]);
            }
