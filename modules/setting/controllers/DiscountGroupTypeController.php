...
class DiscountGroupTypeController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'add-discountgroup', 'add-discountgrouptypedetail'],
                        'roles' => ['@']
                    ],
                    [
                        'allow' => false
    ...
    public function actionView($id)
    {
        ...
        $providerDiscountgroup = new \yii\data\ArrayDataProvider([
            'allModels' => $model->discountgroups,
        ]);
        ...
        return $this->render('view', [
            'model' => $this->findModel($id),
            'providerDiscountgroup' => $providerDiscountgroup,
    ...
    * @return mixed
    */
    public function actionAddDiscountgroup()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('Discountgroup');
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formDiscountgroup', ['row' => $row]);
        } else {
            throw new NotFoundHttpException(Yii::t('app', 'The requested page does not exist.'));
        }
    }
