...
class TimetableController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'pdf', 'save-as-new', 'add-group', 'add-timetable-session'],
                        'roles' => ['@']
                    ],
                    [
                        'allow' => false
    ...
    public function actionView($id)
    {
        ...
        $providerTimetableSession = new \yii\data\ArrayDataProvider([
            'allModels' => $model->timetableSessions,
        ]);
        return $this->render('view', [
            'model' => $this->findModel($id),
            'providerGroup' => $providerGroup,
            'providerTimetableSession' => $providerTimetableSession,
        ]);
    }
    ...
    /**
     * 
     * Export Timetable information into PDF format.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
    * Creates a new Timetable model by another data,
    ...
    */
    ...
    /**
    * Action to load a tabular form grid
    * for TimetableSession
    * @author Yohanes Candrajaya <moo.tensai@gmail.com>
    * @author Jiwantoro Ndaru <jiwanndaru@gmail.com>
    *
    * @return mixed
    */
    public function actionAddTimetableSession()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('TimetableSession');
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formTimetableSession', ['row' => $row]);
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
    }
