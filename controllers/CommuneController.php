...
class CommuneController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'pdf', 'save-as-new', 'add-address', 'add-village'],
                        'roles' => ['@']
                    ],
                    [
                        'allow' => false
    ...
    public function actionView($id)
    {
        ...
        $providerVillage = new \yii\data\ArrayDataProvider([
            'allModels' => $model->villages,
        ]);
        return $this->render('view', [
            'model' => $this->findModel($id),
            'providerAddress' => $providerAddress,
            'providerVillage' => $providerVillage,
        ]);
    }
    ...
    /**
     * 
     * Export Commune information into PDF format.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
    * Creates a new Commune model by another data,
    ...
    */
    ...
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('Village');
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formVillage', ['row' => $row]);
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
    }
